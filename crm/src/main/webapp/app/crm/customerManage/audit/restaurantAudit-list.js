...
angular.module('sbAdminApp')
    .controller('restaurantAuditListCtrl', function ($scope, $rootScope, $http, $filter, $stateParams, $location,$window) {


        $scope.reqType=2; //客户审核
        ...
        $http.get($rootScope.rootPath+"/api/restaurant/reasons").success(function(data) {
        ...
        //})
        ////跟进状态
        //$http.get($rootScope.rootPath+"/api/customerInfo/restaurantSellerType").success(function (data) {
        ...
        $scope.toAudit=function(auditReviewIds , reviewType, reviewStatus, callback){
            ...
            //protected  Integer restaurantReason;
                params: {  restaurantStatus: $scope.auditInfo.restaurantStatus,
                            restaurantReason: $scope.auditInfo.restaurantReason,
                            auditReviewId:auditReviewIds, reviewType: reviewType, reviewStatus: reviewStatus}
            }).success( callback )
                .error(function (data, status, headers, config) {
                    if(data.msg!=null){
                        alert(data.msg);
        ...
        $scope.canAudit=function(){
            ...
            for( var i=0;i<$scope.checkboxSelect.val.length ; i++ ){
                ...
                if(selectVal.infoVo.restaurantStatus!=$scope.undefinedStatus){
                    return false;
                };
            }
            return true;
